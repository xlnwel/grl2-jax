...
from env.overcooked_env2.mdp.overcooked_mdp import OvercookedGridworld, Direction, Action
from env.overcooked_env2.mdp.overcooked_env import OvercookedEnv
from env.overcooked_env2.utils import load_dict_from_file

from env.overcooked_env2.imitation.behavioural_cloning import load_bc_model
...
def setup_game(run_type, run_dir, run_seed, agent_num, player_idx):
    if run_type == "ppo":
        ...
        agent, config = load_bc_model(run_dir)
    else:
...
if __name__ == "__main__" :
    ...
    run_type, run_dir, slow_time, run_seed, agent_num, player_idx = args.type, args.run, bool(args.slow), int(args.seed), int(args.agent_num), int(args.idx)
