...
class UnityEnv:
    ...
    def step(self, actions):
        ...
        done, reward, detect_r = self._get_done_and_reward()
        ...
        self._info = [dict(
            ...
            left_missile_red=np.hstack([self.red_missile_left[i]] * self.n_units),
            left_missile_blue0=np.array([self.blue_missile_left[i][0]] * self.n_units),
            left_missile_blue1=np.array([self.blue_missile_left[i][1]] * self.n_units),
            ...
            main_alive_steps=np.array([self.alive_steps[i][0]] * self.n_units),
            ally_alive_steps=np.array([self.alive_steps[i][1:].mean()] * self.n_units),
            blue_detect_main=np.array([self.detect_steps[i][0]] * self.n_units),
            blue0_alive_steps=np.array([self.alive_steps[i][-2]] * self.n_units),
            blue1_alive_steps=np.array([self.alive_steps[i][-1]] * self.n_units)
        ) for i in range(self.n_envs)]
            ...
            if done[i] == 0:
                for j in range(len(self._blue_names[0])):
                    if self.alive_units[i][j + 5] == 0:
                        ...
                        self.alive_steps[i][j + 5] = self._epslen[i]

                for j in range(self.n_red_main, self.n_units):
                    reward[i][j] += self.reward_config['detect_reward'] if self.detect_units[i][j] else 0
                    detect_r[i][j] += self.reward_config['detect_reward'] if self.detect_units[i][j] else 0
