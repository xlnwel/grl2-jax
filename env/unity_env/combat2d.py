...
class UnityEnv:
    ...
    def reset(self):
        self._epslen = np.zeros(self.n_envs, np.int32)
        self.last_v = np.zeros((self.n_envs, 3), np.float32)
        self.last_angle = np.zeros((self.n_envs, 3), np.float32)
        self._v = np.zeros((self.n_envs, 3), np.float32)
        self._angle = np.zeros((self.n_envs, 3), np.float32)
        self.alive = np.ones((self.n_envs, 1), np.int32)
        self._epslen = np.zeros(self.n_envs, np.int32)
        ...
        self.last_v[0] = obs[0]['obs'][0][0][0:3]
    ...
    def get_obs(self, ds, reward=None, action=None):
        ...
        delta_roll = (angle[2] - self.last_angle[0][2]) / 180
        if abs(delta_roll) > 1:
        ...
        # if abs(v[2]) > 0.3:
        #    print('eps: ' + str(self._epslen))
        #    print('last action:' + str(self._action))
        #    print('last angle: ' + str(self.last_angle[0]))
        #    print('now angle:' + str(angle))
        #    print('last v:' + str(self.last_v[0]))
        #    print('now v:' + str(v))
    ...
    def _get_done_and_reward(self):
        ...
        if self.xyz[0] > 70 or self.xyz[0] < -70 or \
                self.xyz[1] > 15 or self.xyz[2] > 80 or self.xyz[2] < -80:
            done[0] = True
            edge[0] = 1
            return done, reward, score, fail, edge
        if self._height <= LOW_HEIGHT or self.overload > 9 or self.overload < -3:
            done[0] = True
