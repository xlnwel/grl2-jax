...
from env.battle5v5.utils.test_utils import CalNineDir, Trans
...
class AloAgent(Agent):
    ...
    def make_actions(self, actions, parse_msg, side='red'):
        """
            接受Environment传入的动作，生成每个Agent的仿真执行指令
            :params actions Agents policy net outputs
            :params parse_msg 解析后的态势信息
        ...
        if sim_time > 0 and sim_time % 5 == 0:
            for i in range(len(actions)):
                # 当前Agent的仿真ID
                agent_id = parse_msg['red_info'][str(i)]['ID']
                # 对于每个Agent分别产生动作
                ...
                try:
                    last_x, last_y, last_z, last_heading, last_pitch, last_roll = \
                        cur_agent_pre_loc['X'], cur_agent_pre_loc['Y'], cur_agent_pre_loc['Z'], \
                            cur_agent_pre_loc['heading'], cur_agent_pre_loc['pitch'], cur_agent_pre_loc['roll']
                except:
                    # 实体死亡，传过来是
                ...
                # Agent产生攻击动作 9 ~ 13
                    ...
                    if cur_agent_speed <= 0:
                        ...
                        if speed_con_action == 14:
                            speed_change = (9.8 * 1) if agent_id == 1 else (9.8 * 2) # 有人机按照加速度增加9.8 m/s， 无人机增加9.8 * 2 m/s
                        elif speed_con_action == 15:
                            speed_change = (9.8 * (-1)) if agent_id == 1 else (9.8 * (-2))
    ...
    def new_loc(self, x, y, z, heading, pitch, roll, action) -> dict:
        ...
        # 调用utils函数，可以得到9个方向的经纬高（弧度）新坐标
        cal_dir = CalNineDir()
        all_dir_choice = cal_dir.get_all_nine_dir(pitch, roll, heading, x, y, z)
        new_point = all_dir_choice[str(int(action))] # 拿到新的（纬度、经度、高度）
    ...
    def make_init_cmd(self):
        ...
        INIT_LOC = {
            ...
            '1': {'X': -128000, 'Y': -4500, 'Z': 9000},
            '2': {'X': -125000, 'Y': -4500, 'Z': 9000},
            '3': {'X': -128000, 'Y': -5000, 'Z': 9000},
            '4': {'X': -125000, 'Y': -5000, 'Z': 9000},
        }
        global fly_config
        cmd_list = []
        for ia in range(5):
    ...
    def make_changes_speed_cmd(self, agent_id, cmd_speed, cmd_accmag=1.0, cmd_g=3.0):
        """
            修改飞机飞行参数指令，改变飞机速度
        """
...
#         cmd_list = []
#         if parse_msg['sim_time'] < 3:
#             leader_id = 0
#             for i in range(len(self.cur_msg['red_info'])):
#                 agent_id = self.cur_msg['red_info'][str(i)]['ID']
#                 if agent_id != 1:
#                     leader_id = agent_id
#                     break
#             for i in range(len(self.cur_msg['red_info'])):
...
#                     }
#                 }
#                 if agent_id == leader_id: # 领队机跟随敌军有人机
#                     target_ID = self.cur_msg['blue_info']["0"]['ID']
#                     fly_param = fly_config['0']
#                 elif agent_id == 1: # 有人机
#                     target_ID = self.cur_msg['red_info'][str(leader_id)]['ID']
#                     fly_param = fly_config['2']
#                 else: # 剩余跟随有人机
#                     target_ID = self.cur_msg['red_info'][str(leader_id)]['ID']
#                     fly_param = fly_config['1']
#                 follow_cmd = self.make_follow_cmd(agent_id, target_ID, fly_param)
#                 cmd_list.append(follow_cmd)
#         if parse_msg['sim_time'] < 260:
#             return cmd_list
#         for i in range(len(actions)):
#             # 当前Agent的仿真ID
#             agent_id = parse_msg['red_info'][str(i)]['ID']
...
#     def make_follow_cmd(self, agent_id, target_id, fly_param):
#         """
#             生成跟随指令
#             :param agent_id: 当前Agent ID
#             :param target_id: 跟随目标ID
...
#     def make_areapatrolparam_cmd(self, agent_id, new_point):
#         """
#             生成区域巡逻指令, 当智能体输出对自己的跟随指令时，则转为执行区域巡逻指令
#             :param receiver: Handle ID
#             :param x: 区域中心坐标x坐标
