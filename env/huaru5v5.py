...
from env.battle5v5.env.environment import HuaRuBattleEnvWrapper
from env.battle5v5.config import ADDRESS, config, POOL_NUM, ISHOST, XSIM_NUM
from env.battle5v5.env.env_cmd import CmdEnv
from env.battle5v5.env.env_runner import EnvRunner
from env.utils import *
from env.battle5v5.env.env_cmd import CmdEnv
from env.battle5v5.utils.test_utils import CalNineDir
...
class HuaRu5v5(HuaRuBattleEnvWrapper):
  def __init__(
    self,
    env_name, 
    eid, 
    ...
    **kwargs
    ...
    super().__init__(self, address=address, mode=mode)
    self.abundant_info = abundant_info
    ...
    self.observation_space = [{
      'obs': gym.spaces.Box(high=float('inf'), low=0, shape=(152 if self.abundant_info else 150,)),
      'global_state': gym.spaces.Box(high=float('inf'), low=0, shape=(364 if self.abundant_info else 344,)), 
  ...
  def reset(self, if_test=False, args=None, cur_time=None):
    ...
    for side, agent in self.agents.items():
      agent.reset()
    EnvRunner.reset(self)
    msg = EnvRunner.step(self, [])
    ...
    if msg["sim_time"] == 0.0:
      msg = EnvRunner.step(self, [])  # 推动拿到第一帧的obs信息
    ...
    self.last_msg = msg
    self.msg = EnvRunner.step(self, cmd_list)
    self.update_is_locked(self.msg)
    self.update_alive_mask(self.msg)
  ...
  def step(self, actions):
    ...
    """
    last_msg, msg = self.take_action(actions)
    self.update_is_locked(msg)
    self.update_alive_mask(msg)
    obs = self.make_obs(msg)
    reward = self.get_reward(last_msg, msg)
    done, done_reward, done_reason = self.get_done(msg)
    # self.reward计算的是累积奖励，没有什么用, cur_reward才是当前帧的奖励
    self._dense_score += reward + done_reward
    ...
    info = {
      ...
      'red_left_missile': sum([p['LeftWeapon'] for p in msg[Agent.RED]['platforminfos']]), 
      'blue_left_missile': sum([p['LeftWeapon'] for p in msg[Agent.BLUE]['platforminfos']]), 
  ...
  def take_action(self, action):
    ...
    cmd_list.extend(self.agents[Agent.RED].make_actions(action, parse_msg_)) # Agents的仿真指令
  ...
  def check_alive(self, msg, i, side):
    ...
    uid = (RED_INFO if side == Agent.RED else BLUE_INFO)[str(i)]['ID']
  ...
  def get_obs(self, msg, uid):
    ...
    if not self.red_alive_mask[uid]:
      ...
      self.red_agent_loc[str(uid)] = {
        'X': None, 'Y': None, 'Z': None, 
        'heading': None, 'pitch': None, 'roll': None, 
      }
      self.agents_speed[str(uid)] = 0
      return obs
    ...
    obs += [x, y, z, heading, speed, leftweapon, is_locked, *ptype_oh]
    ...
    self.agents_speed[str(uid)] = speed
    ...
    for i in range(5):
      ...
      if self.red_alive_mask[i]:
        # 说明这个队友还活着，拿到这个队友的索引
        al_id = get_info_id(pinfos, RED_INFO[str(i)]['ID'])
    ...
    for i in range(5):
      if self.blue_alive_mask[i]:
        # 说明这个敌人还活着
        e_id = get_info_id(pinfos, BLUE_INFO[str(i)]['ID'])
    ...
    if minfos == []:
      ...
      for i in range(12):
        # 一共是12枚导弹，
        # 分别查看这12枚导弹是否出现了
        minfo = list(filter(lambda x: x['Name'] == BLUE_FIRE_INFO[str(i)]['Name'], minfos))
        if not minfo:
          obs += [0.] * 5
          continue
        minfo = minfo[0]
        # 如果出现了，查看是否锁定了自己？
        if minfo['EngageTargetID'] == RED_INFO[str(uid)]['ID']:
          # 说明这枚弹已经出现了
          # 拿到这枚弹的信息
          # 锁定了自己
          m_x = minfo['X'] - x
  ...
  def get_global_state(self, msg, uid):
    def get_ego_info(msg, uid):
      ...
      gs = [x, y, z, heading, speed, leftweapon, is_locked, *ptype_oh]
      if self.abundant_info:
        gs += [pitch, roll]
      return gs, x, y, z
    ...
    def get_info(msg, side, x, y, z):
      ...
      for i in range(5):
        ...
        if alive_mask[i]:
          # 说明这个队友还活着，拿到这个队友的索引
          pid = get_info_id(pinfos, info_dict[str(i)]['ID'])
          ptype = pinfos[pid]['Type']
          ptype_oh = [0, 0]
          ptype_oh[ptype-1] = 1
          px = pinfos[pid]['X'] - x
  ...
  def get_avail_actions(self, msg, uid):
    ...
    # pid = next(((j, item) for j, item in enumerate(self.ori_message[Agent.RED]['platforminfos']) if RED_INFO[str(i)]['ID'] == self.ori_message[Agent.RED]['platforminfos'][j])['ID'], (-1, None))[0]
    # next的正确用法。
    pid = get_info_id(pinfos, RED_INFO[str(uid)]['ID'])
  ...
  def get_reward(self, last_msg, msg):
    ...
    rewards['distance'] = self.distance_reward_scale * distance_reward * self.decay_coefficient(self._epslen)   # 1.0是衰减系数
    ...
    for rid in range(self.n_units):
      if not self.red_alive_mask[rid]:
        continue
      # 当前的坐标
      cur_pid = get_info_id(red_pinfos, RED_INFO[str(rid)]['ID'])
