def linear_interpolation(l_t, l_v, r_t, r_v, t):
  to_float = lambda x: float(x)
  alpha = to_float(t - l_t) / to_float(r_t - l_t)
  return l_v + alpha * to_float(r_v - l_v)


def exponential_interpolation(l_t, l_v, r_t, r_v, t):
  to_float = lambda x: float(x)
...
def stage_interpolation(l_t, l_v, r_t, r_v, t):
  return r_v

schedule_map = {
  'linear': linear_interpolation, 
  'exp': exponential_interpolation, 
  'stage': stage_interpolation
}
...
#           lr = self._interpolation(l_t, l, r_t, r, step, use_jax=True)
#           break
