...
class MetaParams(Module):
    ...
    def __call__(self, name, trainable):
        val = getattr(self, name)
        if trainable and hasattr(self, f'{name}_var'):
            val = getattr(self, f'{name}_var')
...
class H(Module):
    def __init__(self, config, name='meta_params'):
        ...
        self.action_dim = config.pop('action_dim')
        self.hidden_dims = config.pop('hidden_dims')

        for i, h in enumerate(self.hidden_dims):
            setattr(self, f'w{i}', mlp(
