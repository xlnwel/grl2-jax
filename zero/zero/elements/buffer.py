        ...
        if config.adv_type == 'gae':
            adv, traj_ret = \
                compute_gae(
                    ...
                    reset=reset,
                    norm_adv=config.normalize_adv_in_buffer, 
...
class TurnBasedLocalBufferBase(
    ...
    def add_reward_discount(self, eids, uids, reward, discount):
        assert len(eids) == len(uids), (eids, uids)
        for i, (eid, uid) in enumerate(zip(eids, uids)):
            if discount[i, uid]:
                continue
            self._buffers[(eid, uid)]['reward'].append(reward[i])

    def finish_episode(self, eids, uids, reward, discount):
        np.testing.assert_array_equal(discount, 0)
        for eid in eids:
            for uid in uids:
                self._buffers[(eid, uid)]['reward'][-1] = reward[eid][uid]
