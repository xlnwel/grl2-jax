...
class Reward(hk.Module):
    def __init__(
        self, 
        use_next_env_state=False, 
        balance_dimension=False, 
        env_state_embed=None, 
        action_embed=None, 
        out_size=1, 
        name='reward', 
        **config
        ...
        self.use_next_env_state = use_next_env_state
        self.balance_dimension = balance_dimension
        self.env_state_embed = env_state_embed
    ...
    def __call__(self, x, action, next_x):
        if self.balance_dimension:
            env_state_transform, action_transform, net = self.build_net()
            env_state_embed = env_state_transform(x)
            actions = joint_actions(action)
            action_embed = action_transform(actions)
            if self.use_next_env_state:
                next_env_state_embed = env_state_transform(next_x)
                x = jnp.concatenate([env_state_embed, action_embed, next_env_state_embed], -1)
            else:
                x = jnp.concatenate([env_state_embed, action_embed], -1)
        else:
            net = self.build_net()
    ...
    def build_net(self):
        if self.balance_dimension:
            env_state_transform = mlp(out_size=self.env_state_embed)
            action_transform = mlp(out_size=self.action_embed)
            net = mlp(**self.config, out_size=self.out_size)
            return env_state_transform, action_transform, net
        else:
            net = mlp(**self.config, out_size=self.out_size)
            return net
