...
from algo.sacar.run import run
...
class BaseWorker(BaseAgent):
    ...
    @tf.function
    def _compute_priorities(self, state, action, n_ar, reward, next_state, done, steps):
        gamma = self.buffer.gamma
        value = self.value.train_value(state, action, n_ar)
        next_action, next_n_ar = self.actor.train_action(next_state)
        next_value = self.target_value.train_value(next_state, next_action, next_n_ar)
