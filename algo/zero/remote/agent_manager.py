...
class AgentManager:
    ...
    """ Version Control """
    def get_version(self, wait=True):
        ids = [a.get_version.remote() for a in self.agents]
        return self._wait(ids, wait)

    def increase_version(self, wait=False):
        ids = [a.increase_version.remote() for a in self.agents]
        self._wait(ids, wait)

    """ Stats Retrieval """
    def get_config(self, wait=True):
        ids = [a.get_config.remote() for a in self.agents]
        return self._wait(ids, wait)
    ...
    def set_model_weights(self, model_weights, wait=False):
        ids = [a.set_model_weights.remote(mw) 
            for a, mw in zip(self.agents, model_weights)]
        self._wait(ids, wait)

    """ Communications with Parameter Server """
    def publish_strategies(self, wait=False):
        ids = [a.publish_strategy.remote(wait=wait) for a in self.agents]
        self._wait(ids, wait)
