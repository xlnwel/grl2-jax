...
def get_data_format(env, batch_size, sample_size=None, 
    ...
    if is_per:
        data_format['IS_ratio'] = ((batch_size, sample_size), dtype)
        data_format['idxes'] = ((batch_size, sample_size), tf.int32)
    if store_state:
...
def main(env_config, model_config, agent_config, replay_config):
    ...
    data_format = get_data_format(
        ...
        model_config['q']['lstm_units'])
    
    process = functools.partial(process_with_env, env=env)
    dataset = Dataset(replay, data_format, process_fn=process, obs_range=[0, 1])
