...
from core.elements.buffer import Buffer as BufferBase
...
class LocalBuffer(BufferBase):
    def __init__(
        ...
        self.aid = aid
        self.n_units = env_stats.n_units
...
class Buffer(BufferBase):
    ...
    def _sample(self, sample_keys=None):
        ...
        if len(self._queue):
            sample2 = self._queue.popleft()
            sample = batch_dicts([sample, sample2], func=lambda x: np.concatenate(x, 0))
