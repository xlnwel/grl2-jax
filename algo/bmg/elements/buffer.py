...
class TurnBasedLocalBuffer(Buffer):
    ...
    def retrieve_all_data(self):
        ...
        assert data['discount'][-1] == 0, data['discount'][-1]
        for k, v in data.items():
            assert v.shape[0] == self._memlen, (v.shape, self._memlen)
            v = v[-self.maxlen:]
            v = v.reshape(self.n_envs, self.sample_size, *v.shape[1:])
            if k in self._obs_keys:
                extra_v = np.concatenate([
                    v[1:, :1], np.zeros((1, 1, *v.shape[2:]), np.float32)])
                v = np.concatenate([v, extra_v], 1)
            data[k] = v
        self.reset()
...
class ACBuffer(Buffer):
    ...
    def _add_attributes(self, env_stats, model):
        ...
        if self.config.K:
            self.sample_keys.add('meta_param_stats')
    ...
    def merge_data(self, rid: int, data: dict, n: int):
        ...
        # assert set(data) == set(self.sample_keys), (set(data), set(self.sample_keys))
        for k, v in data.items():
        ...
        if self._current_size >= self.max_size:
            data = {k: np.concatenate(
                ...
                )[-self.batch_size:] for k in data.keys()
            }
    ...
    def merge_episode_data(self, rid: int, data: dict, n: int):
        """ Merging Data from Other Buffers """
        # assert set(data) == set(self.sample_keys), (set(data), set(self.sample_keys))
        for k, v in data.items():
            self._buffers[rid][k].append(v)
        ...
        if self._current_size >= self.max_size:
            for k in data.keys():
                print_dict_info([b[k] for b in self._buffers.values()])
                v = np.concatenate([b[k] for b in self._buffers.values()])
                v = v[-self.max_size:]
                v = v.reshape(self.n_envs, self.n_steps, *v.shape[1:])
                if k in self._obs_keys:
                    extra_v = np.concatenate([
                        v[1:, :1], np.zeros((1, 1, *v.shape[2:]), np.float32)])
                    v = np.concatenate([v, extra_v], 1)
    ...
    def _move_data_to_queue(self, data):
        self.reward_history.append(data['reward'])
        reward = np.stack(self.reward_history).mean().reshape(1, 1)
    ...
    def sample(self):
        ...
        if self.config.inner_steps:
            ...
            if len(self._memory) == self.config.inner_steps:
                sample = batch_dicts(self._memory)
                self._memory = []
            assert len(self._memory) <= self.config.inner_steps
