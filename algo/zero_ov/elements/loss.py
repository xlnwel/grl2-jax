...
class Loss(LossBase):
    def loss(
        ...
        stats.opt_value, _ = compute_values(
            self.modules.opt_value, 
            theta.opt_value, 
            rngs[0], 
            data.global_state, 
            data.next_global_state, 
        ...
        opt_value_loss, stats = compute_opt_vf_loss(
            self.config, 
            stats, 
            value=stats.opt_value, 
            v_target=stats.v_target, 
            sample_mask=data.sample_mask, 
            n=data.n
        )
        
        loss = actor_loss + stats.kl_loss + value_loss + opt_value_loss
        stats.loss = loss
    ...
    def opt_loss(
        ...
        stats.opt_value, next_opt_value = compute_values(
            self.modules.opt_value, 
            theta.opt_value, 
            rngs[0], 
            data.global_state, 
        ...
        v_target, stats.raw_adv = jax_loss.compute_target_advantage(
            ...
            discount=data.discount, 
            reset=data.reset, 
            value=lax.stop_gradient(stats.opt_value), 
            next_value=next_opt_value, 
            ratio=lax.stop_gradient(stats.ratio), 
            gamma=stats.gamma, 
            lam=stats.lam, 
            axis=1
...
def compute_vf_loss(
    ...
    if value_loss_type == 'huber':
        ...
        raw_value_loss = .5 * (value - v_target)**2
...
def compute_opt_vf_loss(
    ...
    delta = v_target - value
    raw_value_loss = .5 * delta**2 * jnp.where(delta > 0, 1, config.alpha)
    stats.raw_v_loss = raw_value_loss
    scaled_value_loss, value_loss = jax_loss.to_loss(
        raw_value_loss, 
        coef=stats.value_coef, 
