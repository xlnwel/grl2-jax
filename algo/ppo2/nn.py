...
class PPO(Ensemble):
    ...
    @tf.function
    def action(self, x, state, mask, evaluation=False, prev_action=None, reward=None):
        assert x.shape.ndims % 2 == 0, x.shape
        x, state = self._encode(x, state, mask, 
            prev_action=prev_action, reward=reward)
    ...
    @tf.function
    def compute_value(self, x, state, mask, prev_action=None, reward=None):
        x, state = self._encode(x, state, mask, 
            prev_action=prev_action, reward=reward)
        value = self.value(x)
    ...
    def _encode(self, x, state, mask, prev_action=None, reward=None):
