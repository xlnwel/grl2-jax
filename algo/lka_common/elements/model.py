...
class LKAModelBase(MAModelBase):
    ...
    def switch_prev_params(self, aids=None):
        if aids is None:
            aids = np.arange(self.n_agents)
        for i in aids:
            self.params.policies[i], self.prev_params.policies[i] = \
                self.prev_params.policies[i], self.params.policies[i]
    ...
    def compute_policy_distances(
        self, data, stats=AttrDict(), eval_lka=True
    ):
        self.pd_rng, rng = random.split(self.pd_rng)
        return self._compute_policy_distances(
            self.prev_params, self.prev_lka_params, self.theta, 
            rng, data, stats, eval_lka
        )

    @partial(jax.jit, static_argnums=[0, 7])
    def _compute_policy_distances(
        self, mu_params, lka_params, pi_params, 
        rng, data, stats=AttrDict(), eval_lka=True
    ):
        ...
        pi_dist = self.joint_policy(
            pi_params.policies, rng, data
        )
        
        mu_dist = self.joint_policy(
            mu_params.policies, rng, data
        )
