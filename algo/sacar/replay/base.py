...
from algo.sacar.replay.utils import *
...
class Replay(ABC):
    ...
    def _get_samples(self, indexes):
        state = np.asarray([np.array(self.memory['state'][i], copy=False) for i in indexes], dtype=np.float32)
        action = np.asarray([np.array(self.memory['action'][i], copy=False) for i in indexes], dtype=np.float32)
        n_ar = np.asarray([np.array(self.memory['n_ar'][i], copy=False) for i in indexes], dtype=np.int32)
        reward = np.asarray([self.memory['reward'][i] for i in indexes], dtype=np.float32)
        done = np.asarray([self.memory['done'][i] for i in indexes], dtype=np.float32)
        steps = np.asarray([self.memory['steps'][i] for i in indexes], dtype=np.float32)
        ...
        if 'next_state' in self.memory:
            ...
            next_indexes = ((indexes + steps) % self.capacity).astype(np.int32)
        ...
        return state, action, n_ar, reward, next_state, done, steps
