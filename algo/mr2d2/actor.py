...
class Worker:
    ...
    def __init__(self, 
        ...
        self.buffer = buffer_fn(buffer_config, state_keys=['h', 'c'], 
            prev_action=0, prev_reward=0, mask=1)
    ...
    @tf.function
    def action(self, obs, state, mask, prev_action, prev_reward):
        qs, state = self.q.value(obs, state, mask, prev_action, prev_reward)
        qs = tf.squeeze(qs)
    ...
    @tf.function
    def _compute_priorities(self, obs, prev_action, prev_reward, discount, logpi, state, q, mask):
        ...
        if self._additional_input:
            ...
            pa, pr = None, None
        x, _ = self.q.rnn(embed, state, mask,
...
class Evaluator(BaseEvaluator):
    ...
    def __init__(self, 
        ...
        self._prev_action = None
        self._prev_reward = None
        self._mask = tf.ones(self.n_envs)
