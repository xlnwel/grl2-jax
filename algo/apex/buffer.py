...
class EnvBuffer(LocalBuffer):
    ...
    def __init__(self, config):
        ...
        self._seqlen = seqlen = config['seqlen']
    ...
    def add_data(self, **data):
        ...
        if self._memory == {}:
            ...
            print(f'Local bufffer keys: {list(self._memory.keys())}')
...
class EnvVecBuffer:
    ...
    def __init__(self, config):
        self._type = config['type']
        self._n_envs = n_envs = config['n_envs']
        assert n_envs > 1
        self._seqlen = seqlen = config['seqlen']
        self._n_steps = config['n_steps']
        self._gamma = config['gamma']
    ...
    def add_data(self, env_ids=None, **data):
        ...
        if self._memory == {}:
            # initialize memory
            init_buffer(self.memory, pre_dims=(self.n_envs, self._seqlen + self.n_steps), 
