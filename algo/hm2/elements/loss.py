...
class MAPPOPolicyLoss(Loss):
    def loss(
        ...
        with tf.GradientTape() as tape:
            x, _ = self.model.encode(obs, prev_reward, prev_action, state, mask)
        ...
        if hasattr(self.model, 'rnn'):
            terms.update(dict(
                ...
                policy_rnn_out=self.model.rnn_out,
                # policy_gate_out=self.model.gate_out,
                # policy_gated_rnn_out=self.model.gated_rnn_out,
...
class MAPPOValueLoss(PPOLossImpl):
    def loss(
        ...
        old_value = value
        with tf.GradientTape() as tape:
            x, _ = self.model.encode(global_state, prev_reward, prev_action, state, mask)
            value = self.value(x)
        ...
        if hasattr(self.model, 'rnn'):
            terms.update(dict(
                ...
                value_rnn_out=self.model.rnn_out,
                # value_gate_out=self.model.gate_out,
                # value_gated_rnn_out=self.model.gated_rnn_out,
