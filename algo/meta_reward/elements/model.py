...
class Model(ModelBase):
    def build_nets(self):
        ...
        self.params.meta_reward, self.modules.meta_reward = build_fn(
            rngs[3], data.global_state, action, get_hx(data.idx, data.event), 
            name='meta_reward')
        self.params.meta_params, self.modules.meta_params = build_fn(
            rngs[4], True, name='meta_params')
        if self.config.use_rnd:
            self.params.rnd, self.modules.rnd = build_fn(
                rngs[5], ov_x, name='rnd'
            )
    ...
    def phi(self):
        ...
        if self.config.use_rnd:
            phi.append('rnd')
        return self.params.subdict(*phi)
    ...
    def compute_eval_terms(self, params, rng, data):
        ...
        if self.config.K:
            _, stats = self.compute_meta_reward(
                params, 
                rng, 
                data.prev_global_state, 
                data.global_state, 
                data.action, 
                idx=data.prev_idx, 
                next_idx=data.idx, 
    ...
    def compute_meta_reward(
        ...
        hx=None, 
        next_hx=None, 
        shift=False,    # whether to shift global_state/idx/event by one step. If so action is at the same step as the next stats, 
        reward=None
    ):
        if hx is None:
            hx = get_hx(idx, event)
        ...
        if self.config.meta_reward_type == 'shaping':
            phi, stats = self.modules.meta_reward(
                eta.meta_reward, rngs[1], global_state, hx=hx)
            next_phi, _ = self.modules.meta_reward(
                eta.meta_reward, rngs[1], next_global_state, hx=next_hx)
            meta_reward = self.config.gamma * next_phi - phi
        elif self.config.meta_reward_type == 'intrinsic':
            x = next_global_state if shift else global_state
            meta_reward, stats = self.modules.meta_reward(
...
def setup_joint_objective(config):
    ...
    if config.joint_theta_objective:
        assert config.K == 0 or config.K is  None, config.K
...
if __name__ == '__main__':
    ...
    pwc(hk.experimental.tabulate(model.raw_action)(
        model.params, model.act_rng, data), color='yellow')
