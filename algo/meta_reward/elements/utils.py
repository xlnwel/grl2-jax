...
from jax_tools import jax_assert, jax_dist, jax_loss, jax_math, jax_utils
...
def compute_joint_stats(
    ...
    v_target, advantage = jax_loss.compute_target_advantage(
        ...
        reset=reset if reset is None else reset[:, :, 0], 
...
def joint_pg_loss(
    ...
    ent_loss, stats = entropy_loss(stats, sample_mask, act_dist)
...
def individual_pg_loss(
    ...
    sample_mask, _ = jax_utils.split_data(
...
def entropy_loss(
    ...
    scaled_entropy_loss, ent_loss = jax_loss.entropy_loss(
    ...
    stats.entropy_loss = ent_loss
    return ent_loss, stats
