...
def compute_values(
    ...
    if next_x is None:
        value, next_value = jax_utils.split_data(value)
    else:
...
def compute_policy(
    ...
    [x, sid, idx, event], _ = jax_utils.split_data(
        [x, sid, idx, event], 
        [next_x, next_sid, next_idx, next_event]
    )
    hx = get_hx(sid, idx, event)
    act_out = func(params, rng, x, hx=hx, action_mask=action_mask)
    if isinstance(act_out, tuple):
        act_dist = jax_dist.MultivariateNormalDiag(*act_out, rng=rng)
    else:
        act_dist = jax_dist.Categorical(act_out, rng=rng)
