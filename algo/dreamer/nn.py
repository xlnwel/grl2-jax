...
class RSSM(Module):
    ...
    def observe(self, embed, action, state=None):
        ...
        post, prior = static_scan(
            lambda inputs, prev: self.obs_step(*inputs, prev[0]),
    ...
    # @tf.function
    def obs_step(self, prev_action, embed, prev_state):
...
def create_model(model_config, state_shape, action_dim, is_action_discrete):
    ...
    models = dict(
        ...
        reward=Decoder(reward_config, 'normal'),
        value=Decoder(value_config, 'normal'),
    ...
    if term_config is not None:
        models['term'] = Decoder(term_config, 'binary')
