...
def analyze_store(store_map):
    n_learned = sum([w.tag == Tag.LEARNED for w in store_map.values()])
    n_evolved = sum([w.tag == Tag.EVOLVED for w in store_map.values()])
    n = len(store_map)
    return dict(frac_learned=n_learned / n, frac_evolved=n_evolved / n)

def store_weights(store_map, score, tag, weights, eval_times, store_cap):
    store_map[score] = Weights(tag, weights, eval_times)
