...
def create_learner(Learner, name, model_fn, replay, config, model_config, env_config):
    config = config.copy()
    ...
    if tf.config.list_physical_devices('GPU'):
        RayLearner = ray.remote(num_cpus=4, num_gpus=.5)(Learner)
    else:
        RayLearner = ray.remote(num_cpus=4)(Learner)
...
def create_actor(Actor, name, model_fn, config, model_config, env_config):
    ...
    if tf.config.list_physical_devices('GPU'):
        RayActor = ray.remote(num_cpus=2, num_gpus=.5)(Actor)
    else:
