...
class MultiAgentSimRunner(RayBase):
    ...
    def random_run(self):
        ...
        step = 0
        agent_env_outs = self.env_output if self.n_units == 1 else self._divide_outs(self.env_output)
    ...
    def _run_impl_ma(self, video: list=None, rewards: list=None):
        ...
        def step_env(agent_actions):
            ...
            assert action.shape == (self.n_envs, self.n_units), (action.shape, (self.n_envs, self.n_units))
    ...
    def _divide_outs(self, out):
        ...
        for pid, aid in enumerate(self.uid2aid):
        ...
        outs = []
        for n, o, r, d, re in zip(self.n_units_per_agent, agent_obs, agent_reward, agent_discount, agent_reset):
            for k, v in o.items():
                o[k] = np.stack(v, 1)
                assert o[k].shape[:2] == (self.n_envs, n), (o[k].shape, (self.n_envs, n))
            r = np.stack(r, 1)
            d = np.stack(d, 1)
            re = np.stack(re, 1)
            assert r.shape == (self.n_envs, n), (r.shape, (self.n_envs, n))
            assert d.shape == (self.n_envs, n), (d.shape, (self.n_envs, n))
            assert re.shape == (self.n_envs, n), (re.shape, (self.n_envs, n))
    ...
    def _log_for_done(self, output: EnvOutput):
        ...
        if done_env_ids:
            ...
            if self.n_units == 1:
                ...
                assert score.shape == (self.n_units, len(info)), (score.shape, (self.n_units, len(info)))
                dense_score = np.stack(dense_score, 1)
                assert dense_score.shape == (self.n_units, len(info)), (dense_score.shape, (self.n_units, len(info)))
