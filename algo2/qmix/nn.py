...
class QMixer(Module):
    def __init__(self, config, n_units, name='qmixer'):
        super().__init__(name=name)

        config = config.copy()
        self.n_units = n_units
        self.hidden_dim = config.pop('hidden_dim')
        self.w1 = mlp(**config, out_size=n_units * self.hidden_dim, 
            name=f'{self.name}/w1')
    ...
    def call(self, qs, state):
        ...
        tf.debugging.assert_shapes([
            [qs, (B, seqlen, self.n_units)],
            [state, (B, seqlen, None)],
        ])
        qs = tf.reshape(qs, (-1, self.n_units))
        state = tf.reshape(state, (-1, state.shape[-1]))

        w1 = tf.math.abs(self.w1(state))
        w1 = tf.reshape(w1, (-1, self.n_units, self.hidden_dim))
        b = self.b(state)
...
class QMIX(Ensemble):
    ...
    def encode(self, obs, state, online=True):
        ...
        seqlen, n_units = x.shape[1:3]

        tf.debugging.assert_equal(n_units, self.qmixer.n_units)
        x = tf.transpose(x, [0, 2, 1, 3])       # [B, A, S, F]
        x = tf.reshape(x, [-1, *x.shape[2:]])   # [B * A, S, F]
        x = rnn(x, state)
        x, state = x[0], self.State(*x[1:])
        x = tf.reshape(x, (-1, n_units, seqlen, x.shape[-1]))  # [B, A, S, F]
...
def create_components(config, env, n_units, **kwargs):
    return dict(
        ...
        target_qmixer=QMixer(config['q_mixer'], n_units, name='target_qmix'), 
    )

def create_model(config, env, **kwargs):
    return QMIX(config, env, n_units=env.n_units, **kwargs)
...
if __name__ == '__main__':
    ...
    x = tf.random.normal((B, env.n_units, 5))
    action_mask = tf.random.uniform((B, env.n_units, env.action_dim), 0, 2, dtype=tf.int32)
    action_mask = tf.cast(action_mask, tf.bool)
    a = model.compute_utils(x)
    x = tf.keras.Input(shape=(env.n_units, 5))
    action_mask = tf.keras.Input(shape=(env.n_units, env.action_dim), dtype=tf.bool)
