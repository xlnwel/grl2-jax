...
def get_data_format(*, env, replay_config, agent_config, **kwargs):
    ...
    data_format = dict(
        obs=((None, sample_size+1, env.n_units, *env.obs_shape), tf.float32),
        global_state=((None, sample_size+1, *env.global_state_shape), tf.float32),
        action_mask=((None, sample_size+1, env.n_units, env.action_dim), tf.bool),
        episodic_mask=((None, sample_size), tf.float32),
        action=((None, sample_size, env.n_units, *env.action_shape), tf.int32),
...
class Agent(Memory, DQNBase):
    ...
    def _add_attributes(self, env, dataset):
        ...
        self._n_units = env.n_units

    @override(DQNBase)
    def _build_train(self, env):
        # Explicitly instantiate tf.function to avoid unintended retracing
        TensorSpecs = dict(
            obs=((self._sample_size+1, self._n_units, *env.obs_shape), env.obs_dtype, 'obs'),
            global_state=((self._sample_size+1, *env.global_state_shape), env.global_state_dtype, 'global_state'),
            action_mask=((self._sample_size+1, self._n_units, env.action_dim), tf.bool, 'action_mask'),
            episodic_mask=((self._sample_size,), tf.float32, 'episodic_mask'),
            action=((self._sample_size, self._n_units, env.action_dim), tf.float32, 'action'),
    ...
    def _compute_target(self, obs, global_state, action_mask, 
        ...
        tf.debugging.assert_shapes([
            [next_utils, (self._batch_size, self._sample_size, self._n_units, self._action_dim)],
            [next_action, (self._batch_size, self._sample_size, self._n_units, self._action_dim)],
        ])
