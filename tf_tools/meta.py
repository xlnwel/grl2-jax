    ...
    **data,
    ...
    if n_mbs > 1:
        ...
        for i in range(n_mbs):
            ...
            terms['grads_norm'], var_norms, clipped_grads = opt(
                tape, loss, return_grads=True, return_var_norms=True
            )
            terms['clipped_grads_norm'] = [tf.norm(v) for v in clipped_grads.values()]
            terms['total_clipped_grads_norm'] = tf.reduce_sum(tf.stack(terms['clipped_grads_norm']))
