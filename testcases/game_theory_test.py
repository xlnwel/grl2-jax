...
from game.rm import RegretMatching
from game.utils import compute_utility
...
class TestClass:
  ...
  def test_regret_matching(self):
    ...
    if np.all(ans[0] == np.array([1, 0])):
      np.testing.assert_allclose(ans[0], np.array([1, 0]))
      np.testing.assert_allclose(ans[1], np.array([1, 0]))
    elif np.all(ans[0] == np.array([0.4, 0.6])):
      np.testing.assert_allclose(ans[0], np.array([0.4, 0.6]))
      np.testing.assert_allclose(ans[1], np.array([0.4, 0.6]))
    elif np.all(ans[0] == np.array([0, 1])):
      np.testing.assert_allclose(ans[0], np.array([0, 1]))
      np.testing.assert_allclose(ans[1], np.array([0, 1]))
    else:
    ...
    rm = RegretMatching(n, m)
    ans = rm.compute(payoffs, n, m, 10000)
    np.testing.assert_allclose(ans[0], np.array([0.625, 0.375]), 1e-3, 1e-3)
