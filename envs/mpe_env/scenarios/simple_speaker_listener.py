import numpy as np
from envs.mpe_env.core import World, Agent, Landmark
from envs.mpe_env.scenario import BaseScenario
...
class Scenario(BaseScenario):
  def make_world(self, config):
    ...
    # add agents
    world.n_units = config['n_units']  # 2
    assert world.n_units == 2, (
      "only 2 agents is supported, check the config.py.")
    world.agents = [Agent() for i in range(world.n_units)]
