...
from envs.envspec import EnvSpec
...
class MultiAgentEnv(gym.Env):
  ...
  def __init__(self, world, reset_callback=None, reward_callback=None,
    ...
    self.n_units = len(world.policy_agents)
    # scenario callbacks
    ...
    if self.use_global_state:
      self.shared_state_spaces = [spaces.Box(
        low=-np.inf, high=+np.inf, shape=(share_obs_dim,), dtype=np.float32) for _ in range(self.n_units)]
    else:
    ...
    if self.shared_viewer:
      ...
      self.viewers = [None] * self.n_units
    self._reset_render()
  ...
  def step(self, action_n, **kwargs):
    ...
    if self.shared_reward:
      reward_n = reward * np.ones(self.n_units)
