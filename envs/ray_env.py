...
from envs.cls import *
from envs.typing import EnvOutput
from envs.utils import batch_env_output
...
class RayVecEnv:
  def __init__(self, EnvType, config, env_fn=make_env):
    ...
    for i in range(self.n_runners):
      config['seed'] = config.get('seed', 1) + i * self.envsperworker
      config['eid'] = config.get('eid', 1) + i * self.envsperworker
