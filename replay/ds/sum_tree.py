...
class SumTree:
    ...
    def __init__(self, capacity):
        ...
        # ensuring SumTree update will not happen during sampling
    ...
    def batch_update(self, mem_idxes, values):
        ...
        # if there are some i < j with idxes[i] == idxes[j], we only keep idxes[i]
        ...
        while len(idxes) > 0:
            ...
            # idx1 contains the unique indexes of p_idxes from left to right
            idxes, idx1, count = np.unique(p_idxes, return_index=True, return_counts=True)
            # idx2 contains the unique indexes of p_idxes from right to left
