...
class Agent(PPOBase):
    ...
    def _learn(self, obs, action, value, traj_ret, advantage, logpi, state=None, mask=None):
        ...
        with tf.GradientTape() as tape:
            ...
            qs = self.actor.value(x)
            act_dist = self.actor.act_dist(qs)
            ...
            # one_hot = tf.one_hot(action, qs.shape[-1], dtype=qs.dtype)
            # q = tf.reduce_mean(qs * one_hot, axis=-1)
            # q_loss = .5 * tf.reduce_mean((q - traj_ret)**2)
            # loss = actor_loss + q_loss
            loss = actor_loss
            # grads = tape.gradient(loss, self._actor_opt.variables)
            # [v.assign_sub(g) for v, g in zip(self._actor_opt.variables, grads)]
            # x = self.encoder(obs)
            # if state is not None:
            #     x, state = self.rnn(x, state, mask=mask)
            # qs = self.actor.value(x)
            ...
            # policy_loss, entropy, kl, p_clip_frac = ppo_loss(
            #     log_ratio, advantage, self._clip_range, entropy)
            # meta_loss = (policy_loss - self._entropy_coef * entropy)
        # terms['actor_norm'] = self._actor_opt(tape, )
