...
#     results = ray.get([
#         evaluate_config.remote(AttrDict2dict(config), n//n_runners)
...
#     win_pstd = np.std(np.mean(wins, axis=-1)) * np.sqrt(n // n_runners)
#     score_pstd = np.std(np.mean(scores, axis=-1)) * np.sqrt(n // n_runners)
#     score2_pstd = np.std(np.mean(scores2, axis=-1)) * np.sqrt(n // n_runners)
